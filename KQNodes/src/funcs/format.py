...
from src.constants.base import BASE_TWO, COLON_DELIM, VOID_STR, EFFECT, ACTUAL
...
def fmt_k_particion(grupos: list[tuple[list[int], list[int]]]) -> str:
    ...
    PORTADO desde el arbol KGeoMIP (Method2), adaptado al `fmt_parte_q` de este
    arbol (estilo ⎛...⎞). Generaliza `fmt_biparticion_q` a k grupos.
    ...
    Returns:
        str con la k-particion en el mismo estilo visual que fmt_biparticion_q.
    """
    tops, bottoms = [], []
    for futuros, presentes in grupos:
        nodos = [(EFFECT, int(f)) for f in futuros] + [(ACTUAL, int(p)) for p in presentes]
