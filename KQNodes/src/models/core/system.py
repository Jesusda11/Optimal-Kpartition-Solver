...
class System:
    ...
    def k_bipartir(self, grupos: list) -> "System":
        """
        Aplica una k-particion de nodos al sistema (generalizacion de `bipartir`).

        Portado desde el arbol KGeoMIP (Method2). Cada grupo i recibe un
        subconjunto de nodos; la variable futura y la variable presente de cada
        nodo van al mismo grupo (definicion S(n,k)). El n-cubo j queda
        marginalmente condicionado solo sobre las variables presentes de su
        propio grupo: marginalizar(setdiff(dims, presentes_i)).

        A diferencia de `bipartir` (k=2), aqui la particion se especifica como una
        lista explicita de grupos, lo que permite k >= 2 arbitrario. Reutiliza la
        misma primitiva `NCube.marginalizar`, por lo que es consistente con la
        evaluacion de phi del arbol QNodes (`distribucion_marginal` + `emd_efecto`).

        Args:
            grupos: lista de (futuros, presentes) donde cada elemento son
                    arrays/listas de indices reales de nodos del grupo.
        ...
        nuevo_sistema.ncubos = tuple(
            cubo.marginalizar(
                np.setdiff1d(cubo.dims, presentes_por_indice[int(cubo.indice)])
            )
            for cubo in self.ncubos
        )
        return nuevo_sistema
